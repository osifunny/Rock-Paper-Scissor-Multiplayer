...
public class CardNetPlayer : MonoBehaviourPun
{
    ...
    public void Set(CardPlayer player){
        ...
        foreach (var card in cards){
            ...
            if(!photonView.IsMine) button.interactable = false;
        }
    }

    private void RemoteClickButton(Attack value){
        if(photonView.IsMine) photonView.RPC("RemoteClickButtonRPC", RpcTarget.Others, (int) value);
    }

    [PunRPC] private void RemoteClickButtonRPC(int value){
        foreach (var card in cards){
